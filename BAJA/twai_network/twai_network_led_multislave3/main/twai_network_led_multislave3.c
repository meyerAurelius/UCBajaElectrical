...
#define ID_SLAVE_STOP_RESP              0x0D0
#define ID_SLAVE_DATA                   0x0D1
#define ID_SLAVE_PING_RESP              0x0D2
            ...
            while (1) {
                ...
                if (rx_msg.identifier == ID_MASTER_START_CMD &&
                    rx_msg.data_length_code >= 4) {
                        ...
                        uint8_t r    = rx_msg.data[1];
                        uint8_t g    = rx_msg.data[2];
                        uint8_t b    = rx_msg.data[3];
                        // Only react if this frame is for us or broadcast
                        if (addr == MY_SLAVE_ADDR) {
                            start_r = r;
                            start_g = g;
                            start_b = b;

                            ESP_LOGI(EXAMPLE_TAG,
                                "Received START for addr=0x%02X (me=0x%02X), color R=%u G=%u B=%u",
                                addr, MY_SLAVE_ADDR, r, g, b);

                            xSemaphoreGive(ctrl_task_sem);
                            break;  // proceed with LED + STOP flow
                        } else{
                            // Not for us, ignore and keep listening
                            ESP_LOGD(EXAMPLE_TAG,
                                "Ignoring START for addr=0x%02X (me=0x%02X)",
                                addr, MY_SLAVE_ADDR);
                        }
                    }
            }

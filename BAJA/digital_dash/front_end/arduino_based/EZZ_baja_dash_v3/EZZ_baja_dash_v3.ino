...
#define TFT_HOR_RES 240 //240
#define TFT_VER_RES 320 //320
...
void setup() {
  ...
  touchscreen.setRotation(2);            //changed from 3 to 2               /* Inverted landscape orientation to match screen */
  ...
  // Rotate the whole LVGL display so the speedometer is horizontal
...
void loop() {
  ...
  */
  ...
  // Map 0-65 km/h -> 0-180 degrees of sweep
  int angle_deg = map(value, 0, 65, 0, 180);

  //Set ARC value
  lv_arc_set_value(objects.gradient_arc, angle_deg); //arc_rainbow

  //Set image rotation so needle moves with the ARC
  lv_image_set_rotation(objects.speed_needle, angle_deg * 10); //img_needle
  //lv_image_set_src(objects.img_needle, "C:\Users\kolap\Documents\Arduino\_images\_MyDesigns\NeedleImagesRotated\needle_108.png");

  //Update label values
  lv_label_set_text_fmt(objects.value_in_speed, "%d", value); //lable_arc_value
  //lv_label_set_text_fmt(objects.lable_needle_value, "Needle: %d", value * 10);

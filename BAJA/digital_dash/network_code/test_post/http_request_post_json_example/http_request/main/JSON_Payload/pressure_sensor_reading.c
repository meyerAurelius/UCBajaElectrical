...
static const char *TAG = "pressure_sensor_reading";

/* ------------------------------------------------------------
 * protected helpers
 * ------------------------------------------------------------ */
static void _pressure_sensor_reading_set_defaults(pressure_sensor_reading_t *reading)
{
	reading->pressure = NAN;
	reading->temperature = NAN;
	reading->raw_value = NAN;
}
...
pressure_sensor_reading_t pressure_sensor_reading(const char *sensor_id)
{
	...
	reading.base.to_json = pressure_sensor_reading_to_json;

	_pressure_sensor_reading_set_defaults(&reading);
...
{
	const pressure_sensor_reading_t *reading;
	cJSON *root;
	cJSON *data;

	reading = (const pressure_sensor_reading_t *)item;
	...
	if (root == NULL)
	{
		ESP_LOGE(TAG, "failed to create pressure json object");
		return NULL;
	}
	...
	{
		cJSON_Delete(root);
		ESP_LOGE(TAG, "failed to create pressure data array");
		return NULL;
	}

	sensor_reading_helpers_add_float_or_null(data, reading->pressure);
	sensor_reading_helpers_add_float_or_null(data, reading->temperature);
	sensor_reading_helpers_add_float_or_null(data, reading->raw_value);

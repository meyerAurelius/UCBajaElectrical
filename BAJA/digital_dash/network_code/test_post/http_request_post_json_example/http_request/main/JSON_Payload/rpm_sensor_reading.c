...
static const char *TAG = "rpm_sensor_reading";
...
rpm_sensor_reading_t rpm_sensor_reading(const char *sensor_id)
{
	...
	reading.base.to_json = rpm_sensor_reading_to_json;
...
{
	const rpm_sensor_reading_t *reading;
	cJSON *root;
	cJSON *data;

	reading = (const rpm_sensor_reading_t *)item;
	...
	if (root == NULL)
	{
		ESP_LOGE(TAG, "failed to create rpm json object");
		return NULL;
	}
	...
	{
		cJSON_Delete(root);
		ESP_LOGE(TAG, "failed to create rpm data array");
		return NULL;
	}

	sensor_reading_helpers_add_float_or_null(data, reading->rpm);
